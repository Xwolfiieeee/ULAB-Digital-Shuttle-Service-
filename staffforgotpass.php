...
if (isset($_POST['resetRequest'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        $sql = "UPDATE staff SET token = ?, expiry = ? WHERE email = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sss", $token, $expiry, $email);
        $stmt->execute();

       
        $resetLink = "http://localhost/signup/staffresetpass.php?token=$token";
        ...
        try {
            ...
            $mail->setFrom('phantomberen@gmail.com', 'Noor E Saba');
...
<head>
    ...
    <link rel="stylesheet" href="staffforgotpass.css">
</head>
<body>
    <header>
